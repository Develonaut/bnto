...
| **Defined param not used in all code paths** | Every param in `metadata()` must be read in `process()` for ALL format/variant branches. Test with different values to verify output changes. See [engine-node-patterns.md](../../scopes/rust/engine-node-patterns.md)                                    |
...
| `.claude/scopes/rust/engine-node-patterns.md`    | **MANDATORY** when adding nodes — full 7-phase checklist, test counts, codegen pipeline |
