...
**Stack context:** Next.js 16.1.6, React 19.2, Turbopack, Tailwind v4, Convex (real-time backend), deployed on Vercel. Monorepo with `transpilePackages` for `@bnto/auth` and `@bnto/core`. The CLI is the primary engine consumer; the web app is a secondary consumer accessing the engine through WASM + `@bnto/core`.
