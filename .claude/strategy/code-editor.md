...
- When the cursor is in a node with `"type": "image-compress"`, the `parameters` object should validate against `imageCompressSchema`
- When in `"type": "spreadsheet-clean"`, validate against `spreadsheetCleanSchema`
