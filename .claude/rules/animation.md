...
**The insight:** When things _appear_, they have a satisfying springy pop -- like a building materializing on the map. Once present, all motion is smooth and calm. Entrances are playful; everything after is serene.
...
## Component API (Animation Components)

**Compose, don't className.** All animation in consumer code MUST use these components. Never apply `motion-safe:animate-*` classes, transition utility classes for entrance effects, or CSS variable overrides directly. The components are the API.

```tsx
import { Stagger, ScaleIn } from "@bnto/ui";

// Stagger cascade with springy cards
<Stagger className="grid grid-cols-3 gap-4">
  {items.map((item, i) => (
    <ScaleIn key={item.id} index={i} from={0.85}>
      <Card>...</Card>
    </ScaleIn>
  ))}
</Stagger>
...
<ScaleIn from={0.5} easing="spring-bouncier">
  <Card>Hero Card</Card>
</ScaleIn>
```
...
| ----------- | ----------------------------- | ---------------------------------------------------- |
| `Stagger`   | `interval?: number \| string` | `stagger-cascade`, `--stagger-interval`              |
| `ScaleIn`   | `from?: number`               | `motion-safe:animate-scale-in`, `--scale-from`       |
| `FadeIn`    | --                            | `motion-safe:animate-fade-in`                        |
| `SlideUp`   | `distance?: number \| string` | `motion-safe:animate-slide-up`, `--slide-distance`   |
| `SlideDown` | `distance?: number \| string` | `motion-safe:animate-slide-down`, `--slide-distance` |
| `PulseSoft` | --                            | `motion-safe:animate-pulse-soft`                     |
...
By default, animation components render a wrapper `<div>`. This is intentional -- opacity animation flattens `transform-style: preserve-3d`, which breaks the `.surface` 3D effect on Cards. The wrapper isolates the animation from the surface element.

Use `asChild` only when the child does NOT use `.surface`:

```tsx
// GOOD -- wrapper div isolates animation from surface
<ScaleIn from={0.85} index={i}>
  <Card>...</Card>
</ScaleIn>
...
1. **Compose with animation components — NEVER use animation classNames directly.** This is the #1 rule. When you need an entrance animation, reach for `<ScaleIn>`, `<SlideUp>`, `<FadeIn>`, etc. Never apply `motion-safe:animate-*` classes, `--scale-from` variables, or `--stagger-index` CSS variables in consumer code. The animation components handle reduced-motion, stagger timing, easing, and surface isolation automatically. Raw animation classes are implementation details — they exist so animation components can use them internally, not for direct consumption. If you catch yourself writing `className="motion-safe:animate-scale-in"` or `className="transition-all duration-fast ... scale-90 opacity-0"`, stop and use the component instead.
2. **Use tokens, not raw values.** `duration-normal` not `duration-300`. `var(--ease-spring)` not inline `linear(...)`.
3. **CSS-powered, component-composed.** The animation system is CSS under the hood (keyframes, transitions, `linear()` springs). But consumers interact with it through animation composition components, not CSS classes. This gives us the performance of CSS with the ergonomics and safety of React composition.
4. **Always guard with motion-safe.** Every animation path must have a reduced-motion fallback. Animation components handle this automatically — another reason to use them instead of raw classes.
...
8. **Only reach for `motion/react` when CSS can't handle it.** Exit animations (unmount), layout animations (reflow), gesture-based interactions (drag/swipe). Everything else is CSS via animation components.
