...
| EN2 | "Open in Editor" on tool page clones recipe to personal store, loads by ID | Enter         | `@editor` | Clone-on-click, recipe loading, all nodes present |
| EN3 | Invalid slug on tool page shows 404 (no editor involvement)                | Enter         | `@editor` | Invalid slug handled at web layer                 |
| EN4 | "Open in Editor" button navigates to `/editor?recipe={id}`                 | Discover      | `@editor` | Template cloned, navigated by ID                  |
...
| **Predefined recipe parity**               | PR1-PR6, EN2          | Clone via "Open in Editor" → load by ID → run → compare output to recipe page execution |
...
| Helper                                    | Purpose                                                                                                |
| ----------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| `navigateToEditor(page, slug?)`           | Navigate to `/editor` (blank) or tool page → "Open in Editor" → `/editor?recipe={id}`, wait for canvas |
