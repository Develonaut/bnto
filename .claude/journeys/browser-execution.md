...
### Verification Convention: 4-Phase Approach

Every browser execution E2E test verifies the full execution lifecycle programmatically:

```
Phase 1: BEFORE    → Page loaded, files selected, ready to run (data attributes)
Phase 2: PROGRESS  → Execution in progress (progressbar visibility, data-phase)
Phase 3: FINISH    → Execution complete, results displayed (output-file count, data-phase="completed")
Phase 4: VERIFY    → Output downloaded and validated (file size, magic bytes, content)
```

Phase 4 (VERIFY) is the primary verification. All phases use programmatic assertions (data attributes, magic bytes, file sizes, download events). Screenshots are reserved for page-level layout verification in `pages/` and `auth/` specs.

### Shared Helpers

All browser journey specs use shared helpers from `apps/web/e2e/helpers.ts`:

| Helper | Purpose |
|---|---|
| `navigateToRecipe(page, slug, h1)` | Navigate + wait for heading |
| `assertBrowserExecution(page)` | Verify `data-execution-mode="browser"` |
| `uploadFiles(page, filePaths[])` | Upload + wait for count text + return run button |
| `runAndComplete(page, options?)` | Click Run + wait for terminal phase |
| `downloadAndVerify(page, options?)` | Download + verify magic bytes/size |
| `downloadAllAsZip(page)` | Download All + verify ZIP magic bytes |
