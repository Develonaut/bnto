...
- [x] `apps/web` — **CRITICAL: Anonymous → password userId preservation (C1-C2).** ConvexHttpClient integration tests proved userId is NOT preserved (new user created on upgrade). Browser E2E tests (`anonymous-conversion.spec.ts`) confirmed the same: userId changes during anonymous→password sign-up in browser context too — `@convex-dev/auth` v0.0.90 creates a new user instead of upgrading. **NOT RESOLVED.** C1-C2 and C3 tests marked `test.fixme()`. Anonymous auth (session creation, tool execution) works fine — the bug is specifically in the upgrade/conversion path. **Fix tracked in Sprint 3 Wave 1 as a blocker.**
...
- [ ] `@bnto/backend` — **BLOCKER: Fix anonymous → password userId preservation.** `@convex-dev/auth` v0.0.90 creates a NEW user on password sign-up from an anonymous session instead of upgrading the existing one. The `createOrUpdateUser` callback in `auth.ts` correctly handles `existingUserId` (patches in-place), but the library never passes it during anonymous→password flows. This means: (1) userId changes on conversion — anonymous user's work (run history, saved state) is orphaned, (2) NavUser shows "Sign In" after sign-up from anonymous session because the Convex client still references the old anonymous session token. **Impact:** Blocks the entire monetization funnel — conversion hooks (Save, History) are meaningless if user data is lost on sign-up. Must be fixed before Stripe (M5). **Approach options:** (a) Custom `createOrUpdateUser` that links anonymous→password by device fingerprint or session cookie, (b) Post-signup migration mutation that merges anonymous user data to the new user doc, (c) Upstream fix in `@convex-dev/auth`. **E2E tests ready:** `anonymous-conversion.spec.ts` C1-C2 and C3 are `test.fixme()` — unfixme them when this is resolved. **See also:** `decisions/auth-evaluation.md`, `journeys/auth.md`.
- [ ] `apps/web` — **Unfixme anonymous conversion E2E tests.** Depends on the blocker above. Once userId preservation is fixed: (1) Remove `test.fixme()` from C1-C2 and C3 in `e2e/journeys/auth/anonymous-conversion.spec.ts`, (2) Remove the FIXME comments explaining the `@convex-dev/auth` limitation, (3) Run the tests and confirm both pass — C1-C2 asserts `upgradedUserId === anonymousUserId`, C3 asserts NavUser shows the new email after conversion, (4) Update the file-level JSDoc that documents the known limitation. Also update the PLAN.md Sprint 2A Wave 5 entry that references this as "NOT RESOLVED."
- [ ] `apps/web` — **Address all FIXME comments across the codebase.** 9 FIXMEs (excluding the 2 in `anonymous-conversion.spec.ts` covered above). Grouped by theme: **(1) Nav architecture refactor** — `Navbar.tsx` (3 FIXMEs: extract magic numbers to constants, replace `pendingHref` useState smell, make navbar hiding composable via layout instead of JS `if (hidden) return null`), `MobileNavMenu.tsx` (2 FIXMEs: extract shared compositional parts between desktop/mobile nav, create shared auth section component), `NavUser.tsx` (1 FIXME: refactor into composable compound component with dot-notation parts — trigger, menu, items). **(2) Middleware** — `middleware.ts` (1 FIXME: research Next.js best practice for middleware naming — `MiddlewareNotFoundError.ts` deprecated, may be `proxy.ts` now). **(3) Providers** — `providers/index.tsx` (1 FIXME: `pathnameRef` pattern is a code smell — research recommended Next.js + `@convex-dev/auth` approach for pathname tracking in providers). Research shadcn-blocks repo for nav patterns. After fixing, grep for remaining FIXME/HACK/XXX and confirm zero results.
...
- [ ] **CLAIMED** `.claude/skills/` — Create persona skill files (rust-expert, go-engineer, frontend-engineer, core-architect, project-manager)
- [ ] **CLAIMED** `.claude/skills/` — Rename `/groom` to `/project-manager`, update content to persona-aware
- [ ] **CLAIMED** `.claude/skills/` — Update `/pickup`, `/code-review`, `/pre-commit` to load relevant persona before execution
- [ ] **CLAIMED** `.claude/skills/` — Test: run `/pickup` on an `[engine]` task and verify Rust persona context is loaded
