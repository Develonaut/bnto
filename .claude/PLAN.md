...
- [ ] **CLAIMED** `engine/crates/tonkotsu` — **Field grouping**: `FieldGroup` wraps fields into named sections. In FullScreenEdit, groups render as navigable pages (next/prev). In DisplayEdit, groups render as visual sections with headers. RED tests: group rendering, page navigation, field-to-group mapping (~5 tests)
