...
- [x] `apps/web` — **Enable canvas interaction**: Upgrade `BentoCanvas` to accept an `interactive` prop. When `true`: `nodesDraggable={true}`, `elementsSelectable={true}`, `panOnDrag={true}`, `zoomOnScroll/Pinch={true}`. Consumer provides `onNodesChange` for controlled position state (RF's change/apply pattern). No edge connections — execution order is positional. When `false`: current read-only showcase behavior (backward compatible).
