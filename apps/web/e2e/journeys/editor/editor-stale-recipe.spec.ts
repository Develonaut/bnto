 ...
 * unknown or deprecated node types (e.g., from old localStorage data,
 * stale cloud sync, or a future schema migration).
 *
...
/** A fixed recipe ID we'll inject into localStorage. */
 ...
 * localStorage data from before a node type was renamed/removed.
 */
...
test.describe("editor stale recipe resilience @browser", () => {
  test("SR1: editor loads without crashing when recipe has unknown node types", async ({
    ...
    // Inject stale recipe into localStorage before navigation
  ...
  test("SR2: unknown node type renders with fallback styling", async ({ page }) => {
    // Inject stale recipe into localStorage
    await page.addInitScript(
      ({ key, payload }) => {
        localStorage.setItem(key, payload);
        localStorage.setItem("bnto-editor-beta-dismissed", "true");
