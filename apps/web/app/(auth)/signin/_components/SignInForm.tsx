...
export function SignInForm({ defaultMode }: SignInFormProps) {
  ...
  // Zustand persist hydrates from localStorage asynchronously. On first render
  // hasAccount may still be false (default). Once hydration completes and
  // hasAccount becomes true, sync the mode — but only if the user hasn't
  // manually toggled and no explicit defaultMode was passed.
  const [userToggled, setUserToggled] = useState(false);
  useEffect(() => {
    if (!defaultMode && !userToggled && auth.hasAccount) setMode("signin");
  }, [auth.hasAccount, defaultMode, userToggled]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      // Clear any stale signout signal — signing in invalidates it.
      // Without this, re-signing in within the 10s TTL window causes
      // useAuth to suppress the fresh session as "signing out".
