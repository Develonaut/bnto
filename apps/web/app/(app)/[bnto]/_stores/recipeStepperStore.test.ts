...
import { createRecipeStepperStore } from "./recipeStepperStore";
...
describe("recipeStepperStore", () => {
  let store: StoreApi<RecipeStepperState>;

  beforeEach(() => {
    store = createRecipeStepperStore(DEFAULT_CONFIG);
  });
  ...
  describe("initial state", () => {
    it("starts with empty files and default per-node config", () => {
      ...
      expect(state.resolvedStep).toBe("idle");
      expect(state.isProcessing).toBe(false);
    });

    it("creates independent config copies from defaults", () => {
      const shared = { "compress-image": { quality: 80 } };
      const store1 = createRecipeStepperStore(shared);
      const store2 = createRecipeStepperStore(shared);
  ...
  describe("setNodeParam", () => {
    ...
    it("preserves other params on the same node", () => {
      const multiStore = createRecipeStepperStore({
  ...
  describe("setDerivedStep", () => {
    it("sets activeStep, resolvedStep, and isProcessing", () => {
      store.getState().setDerivedStep(3, "running");

      expect(store.getState().activeStep).toBe(3);
      expect(store.getState().resolvedStep).toBe("running");
  ...
  describe("factory isolation", () => {
    it("different factory calls produce independent stores", () => {
      const store2 = createRecipeStepperStore({ format: { value: "webp" } });
