...
For browser execution, all node crates compile into a single `.wasm` binary (`bnto-wasm`), loaded by a Web Worker and orchestrated by `@bnto/core`. For desktop, the same crates compile as native Rust — no WASM overhead.
