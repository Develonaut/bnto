...
struct CatalogEnvelope {
    ...
    /// JSON Schema (Draft 2020-12) describing the `.bnto.json` Definition format.
    ///
    /// Any consumer can use this schema to validate Definition files without
    /// reimplementing TypeScript types. Generated by `definition_json_schema()`
    /// in `bnto-core`.
...
pub fn node_catalog() -> Result<String, JsValue> {
    ...
    // --- Step 5: Generate the Definition JSON Schema ---
    // This schema describes the `.bnto.json` format so consumers can
    // validate recipe files without reimplementing TypeScript types.
    let definition_schema = bnto_core::definition_json_schema();

    // --- Step 6: Wrap in an envelope with the format version ---
