// WASM bridge for file operations — `#[wasm_bindgen]` exports that convert
...
fn build_combined_result(output: NodeOutput) -> Result<JsValue, JsValue> {
    ...
    //
    // Extract bytes from FileData. In WASM, all outputs are Bytes variant
    // (Path variant is CLI-only). `into_bytes()` is a no-op move for Bytes.
    let bytes = file
        .data
        .into_bytes()
