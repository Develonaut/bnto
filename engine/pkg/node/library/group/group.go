// Package group provides a container node for executing child node sequentially or in parallel.
//
// The group node is a fundamental building block for complex workflows. It acts as a
// container that can execute other node in two modes:
//
// # Sequential Mode (default)
//
// Child node execute one after another, with each node receiving the accumulated
// context from all previous nodes. This is the most common execution pattern.
//
// Use sequential mode when:
//   - Nodes depend on results from preceding nodes
//   - Order of execution matters
//   - You need deterministic execution flow
//
// # Parallel Mode
//
// Child node execute concurrently, all receiving the same initial context.
// Results from all parallel executions are merged into the output context.
//
// Use parallel mode when:
//   - Nodes are independent (no dependencies)
...
//
// Note: In Phase 1a, the group node validates structure and returns metadata.
// Actual child execution is delegated to the engine (orchestration engine) in Phase 2.
package group
...
const (
	// Sequential executes child node one after another.
	Sequential ExecutionMode = "sequential"

	// Parallel executes child node concurrently.
	Parallel ExecutionMode = "parallel"
)

// Group implements the group node type.
//
// It acts as a container for child node and coordinates their execution
// either sequentially or in parallel.
type Group struct{}

// New creates a new group node instance.
//
// The returned node is stateless and can be reused across multiple executions.
func New() *Group {
	return &Group{}
}
...
//	nodes (required): Array of child node definitions
//	_context (optional): Execution context from previous nodes
//
// In Phase 1a, this method validates the group configuration and returns metadata.
// In Phase 2, the engine will use this metadata to orchestrate actual child execution.
//
// Returns a map containing:
//   - mode: The execution mode used
//   - executed: Number of nodes that would be executed
//   - nodes: The child nodes (for engine to execute)
func (g *Group) Execute(ctx context.Context, params map[string]interface{}) (interface{}, error) {
	...
	// Note: Actual child execution will be done by engine in Phase 2
...
// Verify Group implements node.Executable at compile time
var _ node.Executable = (*Group)(nil)
