// Package loop provides iteration node (forEach, times, while modes).
//
// The loop node enables repeating operations over collections or conditions:
//   - forEach: Iterate over an array/slice, passing each item to nested nodes
...
// Loop implements the loop node for iteration operations.
type Loop struct{}

// New creates a new loop node instance.
func New() *Loop {
	return &Loop{}
}

// Execute runs the loop node with the specified mode.
//
// Parameters:
...
//   - _context: execution context from previous nodes
//
// Returns a map with:
