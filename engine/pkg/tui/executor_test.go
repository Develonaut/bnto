...
func TestExecutorSingleNodeSuccess(t *testing.T) {
	...
	if !strings.Contains(output, "Test Workflow") {
		t.Errorf("Expected output to contain 'Test Workflow', got: %s", output)
	}
}
...
// TestExecutorWithRealEngine tests executor with actual engine execution.
func TestExecutorWithRealEngine(t *testing.T) {
