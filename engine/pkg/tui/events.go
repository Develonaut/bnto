...
func (m Model) updateList(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		switch msg.String() {
		case "enter":
			// Select workflow (only if not in reorder mode)
			if !m.reorderMode {
				if selected, ok := m.list.SelectedItem().(BntoItem); ok {
					m.selectedBnto = selected.FilePath
					return m.runBnto()
				}
			}
		case "s":
		...
		case "esc":
			...
			if m.reorderMode {
				...
				// Save current order
				items := m.list.Items()
				bntoItems := make([]BntoItem, len(items))
				for i, item := range items {
					bntoItems[i] = item.(BntoItem)
				}
				order := extractBntoOrder(bntoItems)
				_ = saveBntoOrder(order) // Ignore errors
				return m, nil
			}
		case "up", "k":
...
func (m Model) updateForm(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	if keyMsg, ok := msg.(tea.KeyMsg); ok {
		if keyMsg.String() == "esc" {
			...
			// Otherwise, go back to workflow list
			m.formStage = 0
			m.currentView = listView
			return m, nil
		}
