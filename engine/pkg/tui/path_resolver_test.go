...
func TestResolvePath_BntoHome(t *testing.T) {
	// Test {{BENTO_HOME}} marker
	testPath := "{{BENTO_HOME}}/bntos/test.json"
	...
	expected := filepath.Join(bntoHome, "bntos", "test.json")
...
func TestResolvePath_Combined(t *testing.T) {
	...
	bntoHome := LoadBntoHome()
	expected := filepath.Join(bntoHome, "MyProject", "files")
	if resolved != expected {
		t.Errorf("Expected %s, got %s", expected, resolved)
	}
}
...
func TestResolvePathsInMap(t *testing.T) {
	...
	// Check KEY1
	expected1 := filepath.Join(bntoHome, "file1")
