...
//
// Theme configuration persistence to ~/.bnto/config/theme.
package tui
...
// LoadSaveDirectory loads the saved workflows directory from disk.
// Returns ~/.bnto as default if no saved value or on error.
func LoadSaveDirectory() string {
	return paths.LoadSaveDirectory()
}

// SaveSaveDirectory saves the workflows directory to disk.
// Creates ~/.bnto directory if it doesn't exist.
