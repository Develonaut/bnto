...
// Context helper functions for adding bnto-specific fields to loggers.
...
//	logger = logger.WithBntoID(logger, "my-workflow")
//	logger = logger.WithNodeID(logger, "node-1")
//
//	// Now all logs will include these fields
//	logger.Info("Executing node")

// WithBntoID adds bnto_id to the logger context.
// The engine uses this to tag all logs from a specific workflow.
//
// The bnto_id helps identify which workflow instance generated a log,
// which is critical for debugging parallel workflow executions.
func WithBntoID(logger *Logger, bntoID string) *Logger {
	return logger.With("bnto_id", bntoID)
}
