// Package main implements simple execution for the run command.
//
// This file contains the executeSimple function which runs bnto workflows
// with simple single-line progress output for non-TTY environments (CI/CD, pipes, etc).
package main

import (
	"context"
	"fmt"
	"os"
	"time"

	"github.com/Develonaut/bnto/pkg/api"
	"github.com/Develonaut/bnto/pkg/engine"
	"github.com/Develonaut/bnto/pkg/node"
)

// executeSimple executes bnto with simple single-line progress (non-TTY mode).
func executeSimple(def *node.Definition) error {
	// Create registry and file logger (always log to file)
	p := api.DefaultRegistry()
	logger, logFile, err := createFileLogger()
	if err != nil {
		printError(fmt.Sprintf("Warning: Failed to create log file: %v", err))
		// Continue without file logging
	}
	if logFile != nil {
		defer logFile.Close()
	}

	// Also log to stdout if verbose
	if verboseFlag && logger != nil {
		logger = createDualLogger(logger)
	}

	// Create engine (nil messenger — no progress output in simple mode)
	eng := engine.New(p, logger)

	// Execute bnto
	ctx, cancel := context.WithTimeout(context.Background(), timeoutFlag)
	defer cancel()

	start := time.Now()
	result, err := eng.Serve(ctx, def)
	duration := time.Since(start)

	if err != nil {
		// Get random error status word
		statusWord := getErrorStatusWord()
		printError(fmt.Sprintf("Oh no! Bnto is %s: %v", statusWord, err))
		os.Exit(1)
	}

	printSuccess(fmt.Sprintf("Delicious! Bnto executed %d nodes successfully in %s",
		result.NodesExecuted, formatDuration(duration)))
	return nil
}
