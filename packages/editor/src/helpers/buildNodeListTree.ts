...
interface NodeListTree {
  /** All top-level nodes in original array order, containers have children nested. */
  entries: NodeListEntry[];
  /** True when no non-IO processing nodes exist. */
  isEmpty: boolean;
  /** Index at which to insert the placeholder (-1 if not needed). */
...
function buildNodeListTree(nodes: BentoNode[], configs: NodeConfigs): NodeListTree {
  ...
  // Place placeholder before the last IO node (Output), mirroring canvas
