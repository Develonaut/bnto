...
/** Accumulate a single event into the per-slug stats map. */
function accumulateEvent(
  bySlug: Map<string, SlugAccumulator>,
  event: ExecutionEvent,
) {
  const entry = bySlug.get(event.slug) ?? {
    totalRuns: 0, completedRuns: 0, failedRuns: 0,
    totalDurationMs: 0, durationCount: 0, lastRunAt: 0,
  };
  ...
  if (event.timestamp > entry.lastRunAt) entry.lastRunAt = event.timestamp;

  bySlug.set(event.slug, entry);
}

/** Convert accumulated stats to sorted output array. */
function toSortedSlugStats(bySlug: Map<string, SlugAccumulator>) {
...
export const aggregateBySlug = query({
  ...
  handler: async (ctx) => {
    ...
    const bySlug = new Map<string, SlugAccumulator>();
    for (const event of events) {
      accumulateEvent(bySlug, event);
    }
    return toSortedSlugStats(bySlug);
  },
});
...
/** Compute summary stats from a list of events. */
function computeSummary(events: { status: string; durationMs?: number }[]) {
  let completedRuns = 0;
  let failedRuns = 0;
