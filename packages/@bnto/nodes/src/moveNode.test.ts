...
describe("moveNode", () => {
  it("updates a node's position", () => {
    ...
    expect(result.definition.nodes![0]!.position).toEqual({ x: 300, y: 400 });
  ...
  it("moves the correct node when multiple nodes exist", () => {
    ...
    // Second node moved
    expect(result.definition.nodes![1]!.position).toEqual({ x: 500, y: 300 });
  });
  ...
  it("works on nested nodes (inside containers)", () => {
    ...
    expect(result.definition.nodes![0]!.nodes![0]!.position).toEqual({
      x: 200,
      y: 150,
    });
  });
